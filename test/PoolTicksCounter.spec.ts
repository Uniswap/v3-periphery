  ...
  TICK_SPACINGS.forEach(TICK_SPACING => {
    ...
    describe(`[Tick Spacing: ${TICK_SPACING}]: tick after is bigger`, async () => {
      it('same tick initialized', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(2));
        expect(result).to.be.eq(0);
      })

      it('same tick not-initialized', async () => {
        await pool.mock.tickBitmap.withArgs(0).returns(0b1100); // 1100
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(1), bitIdxToTick(1));
        expect(result).to.be.eq(0);
      })
      
      it('same page', async () => {
        await pool.mock.tickBitmap.withArgs(0).returns(0b1100); // 1100
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(0), bitIdxToTick(255));
      ...
      it('multiple pages', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(0), bitIdxToTick(255, 1));
      ...
      it('counts ticks to left of start and right of end on same page', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(8), bitIdxToTick(255));
        expect(result).to.be.eq(5);
      })
      ...
      it('counts ticks to left of start and right of end across on multiple pages', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(8), bitIdxToTick(8, 1));
      ...
      it('counts ticks when before and after are initialized on same page', async () => {
        ...
        const startingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(255));
        expect(startingTickInit).to.be.eq(6);
        const endingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(0), bitIdxToTick(3));
        expect(endingTickInit).to.be.eq(1);
        const bothInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(5));
      ...
      it('counts ticks when before and after are initialized on multiple page', async () => {
        ...
        const startingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(255));
        expect(startingTickInit).to.be.eq(6);
        const endingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(0), bitIdxToTick(3, 1));
        expect(endingTickInit).to.be.eq(7);
        const bothInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(5, 1));
      ...
      it('counts ticks with lots of pages', async () => {
        ...
        const bothInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(4), bitIdxToTick(5, 4));
    ...
    describe(`[Tick Spacing: ${TICK_SPACING}]: tick after is smaller`, async () => {
      it('same page', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(255), bitIdxToTick(0));
        expect(result).to.be.eq(2);
      })
      ...
      it('multiple pages', async () => {
        ...
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(255), bitIdxToTick(0, -1));
      ...
      it('counts ticks to right of start and left of end on same page', async () => {
        await pool.mock.tickBitmap.withArgs(0).returns(0b1111000100001111);
        const result = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(255), bitIdxToTick(8));
        expect(result).to.be.eq(4);
      })
      ...
      it('counts ticks when before and after are initialized on same page', async () => {
        await pool.mock.tickBitmap.withArgs(0).returns(0b11111100);
        const startingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(3), bitIdxToTick(0));
        expect(startingTickInit).to.be.eq(2);
        const endingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(255), bitIdxToTick(2));
        expect(endingTickInit).to.be.eq(5);
        const bothInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(5), bitIdxToTick(2));
        expect(bothInit).to.be.eq(3);
      })
      ...
      it('counts ticks when before and after are initialized on multiple page', async () => {
        await pool.mock.tickBitmap.withArgs(0).returns(0b11111100);
        await pool.mock.tickBitmap.withArgs(-1).returns(0b11111100);
        const startingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(2), bitIdxToTick(3, -1));
        expect(startingTickInit).to.be.eq(5);
        const endingTickInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(5), bitIdxToTick(255, -1));
      ...
      it('counts ticks with lots of pages', async () => {
        ...
        await pool.mock.tickBitmap.withArgs(-3).returns(0x0);
        await pool.mock.tickBitmap.withArgs(-4).returns(0b11111100);
        const bothInit = await PoolTicksCounter.countInitializedBitsCrossed(pool.address, bitIdxToTick(3), bitIdxToTick(6, -4));
