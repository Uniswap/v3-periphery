...
describe.only('ChainedOracleLibrary', () => {
    ...
    describe('#getPriceChained', () => {
      ...
      it('reverts when secondsAgo is 0', async () => {
        ...
        //const oracleTick = await oracle.consult(mockObservable.address, secondsAgo)
        expect(oracle.getPriceChained(secondsAgo, mockObservable.address, mockObservable.address)).to.be.revertedWith('BP')
      })
